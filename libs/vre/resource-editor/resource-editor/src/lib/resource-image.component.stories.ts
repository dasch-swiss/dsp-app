...
const makeResource = (overrides: Record<string, unknown> = {}): DspResource => {
  ...
  return {
    res: {
      ...
      label: 'Test Image Resource',
      attachedToProject: 'http://rdfh.ch/projects/test',
      attachedToUser: 'http://rdfh.ch/users/test',
      userHasPermission: 'CR',
      properties: { [Constants.HasStillImageFileValue]: [fileValue] },
      entityInfo: { classes: { 'http://example.org/Thing': { label: 'Thing' } } },
      getValues: (prop: string) => (prop === Constants.HasStillImageFileValue ? [fileValue] : []),
      ...overrides,
    },
    resProps: [],
    incomingAnnotations: [],
...
const meta: Meta<ResourceImageComponent> = {
  title: 'Resource Editor / Resource / Image',
